...
* Set video mode and screen location
 ORG $FF90
 FCB $4C
 ORG $FF98
 FCB $90 turn off color burst, default mode is RGB1
 ORG $FF99
 FCB $3E
 ORG $FF9F
 FCB 0
...
* Load title screen
 ...
 FDB $3233
 ORG SCREEN
 includebin compress/title.bin

* Set palettes to reveal title screen
 ORG $FFB0
 FCB 0,32,2,54,52,38,32,32,34,52,52,53,4,32,32,36
...
* First title screen RGB color set
TITLSET
 FCB 0,32,2,54,52,38,32,32,34,0,52,53,4,32,32,36
...
* Display first title screen
 LEAU NULLSET,PCR	* set all palettes black
 LBSR CLRSET		* so we don't see title screen loading
 INC WINSET
 LBSR CLS		* clear screen
 CLR WINSET
 LBSR VINIT		* init video
 LDD #$3637		* title screen in segments $36, $37
 LBSR DRIVER		* unpack title screen
 LEAU TITLSET,PCR	* set palettes for title screen
 LBSR CLRSET
 LBSR CHIME		* ding!
 LDA #15		* break out of title screen in 15 seconds
 STA SECS		* if the fire button isn't pressed
A@
 LDB #1
 LBSR BUTTON		* wait for fire button
 BNE B@
 TST SECS		* wait for 15 seconds to elapse
 BGT A@
B@

* Display second title screen
 LBSR CLS
...
C@
 LBSR CLRSET
 INC MONTYP
 LDB #$90 turn off color burst for RGB modes
 STB $FF98 video mode register
 BRA INIT
MCMP
 LEAU CMPSET,PCR
 LBSR CLRSET
 LDB #$80 turn on color burst for composite
 STB $FF98 video mode register
INIT
 LBSR GDRAGN
...
VINIT
 ...
 LDB #$90 turn off color burst, default mode is RGB1
 STB $FF98 video mode register
 LDB #$3E 200 lpf
;LDB #$7E 225 lpf?
;LDB #$1E 192 lpf?
...
A@
 ...
 LDD #$3435 display victory dragon
 LBSR DRIVER
...
*restore colorset: 0 composite / 1 RGB1 / 2 RGB2
 LEAU RGBSET,PCR
 LDA MONTYP
 CMPA #1
 BEQ B@
...
* Title screen compressed data goes into segments $36 and $37
 ORG $FFA4
 FDB $3637
 ORG $8000
 include title.asm
...
* Victory dragon compressed data goes into segments $34 and $35
 ORG $FFA4
 FDB $3435
 ORG $8000
 include victory.asm
